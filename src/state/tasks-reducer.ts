...
import {v1} from 'uuid';
import {AddTodolistActionType, RemoveTodolistActionType, SetTotolistsActionType} from './todolists-reducer';
...
export const fetchTasksTC = (todolistId: string) => (dispatch: Dispatch) => {
    todolistsAPI.getTasks(todolistId)
        .then(t => {
            dispatch(setTasksAC(t.data.items, todolistId))
        })
}
...
export const addTaskTC = (todolistId: string, title: string) => (dispatch: Dispatch) => {
    todolistsAPI.createTask(todolistId, title)
        .then(t => {
            dispatch(addTaskAC(t.data.data.item))
        })
}
...
export const updateTaskTC = (todolistId: string, taskId: string, title: string) => (dispatch: Dispatch, getState: () => AppRootStateType) => {
    ...
    if (task) {
        ...
        todolistsAPI.updateTask(todolistId, taskId, model)
            .then(t => {
                dispatch(changeTaskTitleAC(taskId, t.data.data.item.title, todolistId))
            })
    }
}
...
export const updateTaskStatusTC = (todolistId: string, taskId: string, status: TaskStatuses) => (dispatch: Dispatch, getState: () => AppRootStateType) => {
    ...
    if (task) {
        ...
        todolistsAPI.updateTask(todolistId, taskId, model)
            .then(t => {
                dispatch(changeTaskStatusAC(taskId, t.data.data.item.status, todolistId))
            })
    }
}